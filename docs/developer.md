...
SaaAssistantAca/
|-- app/
|   |-- presentation/          # Qt pages/widgets/resources
|   |   |-- views/
|   |   |-- widgets/
|   |   `-- resources/
|   |-- application/           # use cases and orchestration
|   |-- core/                  # task_engine/event_bus/config/observability
|   |-- infrastructure/        # automation + common adapters
|   |-- modules/               # vertical modules (usecase + ui)
|   `-- common/                # shared cross-layer utilities
|-- runtime/
|   |-- appdata/               # persisted user config/runtime data
|   |-- log/                   # logs
|   `-- temp/                  # temp files
|-- docs/
|-- PerfectBuild/
...
让我们先移步到app/infrastructure/automation/下，这个目录就是这个问题的答案
...
的具体实现
*
错别字问题：识别结果通常会因为游戏使用了特殊字体或者字过小导致识别出错，这是ocr模型的问题，毕竟这个模型不是针对这个游戏训练的，此时需要有一套错别字替换机制，这也是`runtime/appdata/ocr_replacements.json`
的作用。在获得ocr结果后先根据这个文件里面的替换规则进行对应的处理替换，而代码作者面对一些错别字的时候往里面新增替换规则就好了

## 如何进行统一的日志管理？

那就要移步至`app/infrastructure/logging/gui_logger.py`了，注意点如下：
...
4. 功能对接，来到这你已经打败99%的大佬了
   来到`app/presentation/views/additional_features.py`
