...
from app.infrastructure.config.app_config import is_non_chinese_ui_language, is_traditional_ui_language
from app.presentation.shared.icon import Icon
from app.infrastructure.vision.matcher import matcher
from app.infrastructure.config.setting import REPO_URL
from app.infrastructure.runtime.paths import LOG_DIR, TEMP_DIR, APPDATA_DIR, ensure_runtime_dirs
from app.infrastructure.events.signal_bus import signalBus
from app.core.event_bus.global_task_bus import global_task_bus
from app.core.observability import AppErrorCode, capture_exception
from app.core.task_engine.hotkey_poller import GlobalHotkeyPoller
from app.application.hotkey.routing import resolve_f8_action, HotkeyAction
from app.application.startup.interface_plan import (
...
from app.utils.updater import get_local_version, get_best_update_candidate
from app.presentation.widgets.custom_message_box import CustomMessageBox
from app.presentation.resources import resource_qrc  # don't delete
...
class MainWindow(FluentWindow, BaseInterface):
    ...
    def _resolve_app_icon() -> QIcon:
        qrc_path = ":/app/presentation/resources/images/logo.png"
        icon = QIcon(qrc_path)
        if not icon.isNull():
            return icon
        ...
        file_candidates = [
            base_dir / "app" / "presentation" / "resources" / "images" / "logo.png",
            base_dir / "app" / "presentation" / "resources" / "images" / "logo.ico",
        ]
    ...
    def _to_traditional_if_needed(self, text):
        if not isinstance(text, str) or not is_traditional_ui_language():
            return text
        try:
            from app.presentation.shared.localizer import _to_traditional
    ...
    def _localize_widget_if_needed(self, widget):
        if widget is None or not is_traditional_ui_language():
            return
        try:
            from app.presentation.shared.localizer import localize_widget_tree_for_traditional
