...
class WaterBombModule:
    ...
    def fight(self):
        ...
        while True:
            ...
            if not self.is_speed_up:
                if not self.auto.find_element('app/presentation/resources/images/water_bomb/speed.png', 'image',
            ...
            # 确认加速后进入主循环
                ...
                if is_player_round:
                    if current_strategy == '':
                        if self.auto.find_element('app/presentation/resources/images/water_bomb/head.png', 'image',
    ...
    def handle_shooting(self, person):
        """处理开枪策略下的逻辑"""
        timeout = Timer(20).start()
        path = f'app/presentation/resources/images/water_bomb/{person}.png'
        while True:
            self.auto.take_screenshot()
            ...
            # 射击敌人
                ...
                if self.have_extra_shoot and self.auto.find_element('追加', 'text', crop=(
                    ...
                    self.bullet_type = -1
                    return True
                if self.auto.find_element('app/presentation/resources/images/water_bomb/gun.png', 'image',
        ...
        while True:
            ...
            if self.auto.ocr_result:
                for ocr_result in self.auto.ocr_result:
                    if '空' in ocr_result[0] or self.auto.find_element('app/presentation/resources/images/water_bomb/blank.png',
                                                                       'image', crop=(
                        ...
                        self.bullet_type = 0
                        return
                    elif '水' in ocr_result[0] or self.auto.find_element('app/presentation/resources/images/water_bomb/live.png',
                                                                         'image', crop=(
                                    1470 / 2560, 1290 / 1440, 1640 / 2560, 1364 / 1440),
                                                                         match_method=cv2.TM_CCOEFF_NORMED,
                                                                         is_log=self.is_log,
    ...
    def select_and_reset(self):
        ...
        while True:
            ...
            if self.auto.find_image_and_count(
                    ...
                    'app/presentation/resources/images/water_bomb/steal_select.png', threshold=self.count_threshold,
            ...
            if not select_flag:
                if top_left and bottom_right:
                    if self.auto.find_element('app/presentation/resources/images/water_bomb/steal_select.png', 'image', crop=(
                        ...
                        if index >= len(items_list):
                            return True
                path = f'app/presentation/resources/images/water_bomb/{items_list[index]}_steal.png'
    ...
    def select_and_steal(self, steal_item):
        """选择物品然后偷"""
        timeout = Timer(20).start()
        path = f'app/presentation/resources/images/water_bomb/{steal_item}_steal.png'
        select_flag = False
    ...
    def use_normal_item(self, item_name):
        ...
        path = f'app/presentation/resources/images/water_bomb/{item_name}.png'
        while True:
            self.auto.take_screenshot()
    ...
    def update_extra_status(self):
        """检查当前对面是否被铐上手铐"""
        if self.auto.find_element('app/presentation/resources/images/water_bomb/is_handcuffs.png', 'image',
                                  threshold=self.template_threshold,
    ...
    def update_hp_and_bullet(self):
        ...
        self.current_player_hp = self.auto.find_image_and_count(player_hp_screenshot,
                                                                'app/presentation/resources/images/water_bomb/hp.png',
                                                                threshold=self.count_threshold, is_log=self.is_log)
        no_hp = self.auto.find_image_and_count(player_hp_screenshot, 'app/presentation/resources/images/water_bomb/no_hp.png',
                                               threshold=self.count_threshold, is_log=self.is_log)
        ...
        is_num = True
        if self.auto.find_element(f'app/presentation/resources/images/water_bomb/q.png', 'image',
        ...
        if is_num:
            for i in range(0, 5):
                if self.auto.find_element(f'app/presentation/resources/images/water_bomb/{i}.png', 'image',
    ...
    def get_items(self, crop):
        ...
        current_items = []
        path_list = [
            'app/presentation/resources/images/water_bomb/advanced_barrel.png',
            'app/presentation/resources/images/water_bomb/unload_puppet.png',
            'app/presentation/resources/images/water_bomb/gem_of_life.png',
            'app/presentation/resources/images/water_bomb/reset_hammer.png',
            'app/presentation/resources/images/water_bomb/handcuffs.png',
            'app/presentation/resources/images/water_bomb/reverse_magic.png',
            'app/presentation/resources/images/water_bomb/hand_of_kaito.png',
            'app/presentation/resources/images/water_bomb/insight_sunglasses.png'
        ]
