...
from app.infrastructure.vision.matcher import matcher
from app.infrastructure.events.signal_bus import signalBus
from app.utils.text_normalizer import normalize_chinese_text
from app.utils.randoms import random_rectangle_point
from app.utils.windows import get_hwnd
from app.infrastructure.automation.input import Input
from app.infrastructure.automation.screenshot import Screenshot
    ...
    def back_to_home(self):
        ...
        while True:
            ...
            if self.find_element('基地', 'text', crop=(
                ...
                return True
            elif self.find_element('app/presentation/resources/images/reward/home.png', 'image', threshold=0.5,
        ...
        try:
            ...
            if len(matches) >= 1:
                ...
                if conf >= threshold or threshold is None:
                    ...
                    if is_log:
                        self.logger.debug(ui_text(f"目标图片：{template.replace('app/presentation/resources/images/', '')} 相似度：{conf:.2f}",
                                                  f"Target image: {template.replace('app/presentation/resources/images/', '')} Similarity: {conf:.2f}"))
                    return top_left, bottom_right, conf
                else:
                    if is_log:
                        self.logger.debug(ui_text(f"目标图片：{template.replace('app/presentation/resources/images/', '')} 相似度：{conf:.2f}，低于{threshold}",
                                                  f"Target image: {template.replace('app/presentation/resources/images/', '')} Similarity: {conf:.2f}, below {threshold}"))
            else:
                if is_log:
                    self.logger.debug(ui_text(f"目标图片：{template.replace('app/presentation/resources/images/', '')} 未找到匹配项",
                                              f"Target image: {template.replace('app/presentation/resources/images/', '')} No match found"))
            if is_show:
            ...
            if is_log:
                if len(matches) > 0:
                    for i in range(len(matches)):
                        ...
                        self.logger.debug(ui_text(f"目标图片：{template.replace('app/presentation/resources/images/', '')} 相似度：{conf:.2f}",
                                                  f"Target image: {template.replace('app/presentation/resources/images/', '')} Similarity: {conf:.2f}"))
                self.logger.debug(ui_text(f"图片{template.replace('app/presentation/resources/images/', '')} 个数为 {len(matches)}",
                                          f"Count of image {template.replace('app/presentation/resources/images/', '')} is {len(matches)}"))
            if is_show:
                for idx, (x, y, w, h, conf) in enumerate(matches):
                    cv2.rectangle(temp,
                                  (x, y),
